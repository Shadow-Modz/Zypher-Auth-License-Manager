import { ActionRowBuilder, ApplicationCommandOptionType, ButtonBuilder, ButtonStyle, ComponentType, EmbedBuilder, ModalBuilder, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle, User } from "discord.js";
	...
	options: [
		...
		},
		{
			...
			options: [
				{
					name: "license",
					description: "The license to which the registered ips/hwid will be cleared",
					type: ApplicationCommandOptionType.String,
					required: true
				}
			]
		...
		if (subCommand === "create") {
			...
			const mainButtons = (state: boolean) => {
				const components = [
					new ActionRowBuilder<ButtonBuilder>().addComponents(
						new ButtonBuilder()
							.setStyle(licenseInfo.Product === null ? ButtonStyle.Secondary : ButtonStyle.Primary)
							.setCustomId("product")
							.setLabel("Product")
							.setDisabled(state)
							.setEmoji("📦"),
						new ButtonBuilder()
							.setStyle(licenseInfo.User === null ? ButtonStyle.Secondary : ButtonStyle.Primary)
							.setCustomId("user")
							.setLabel("User")
							.setDisabled(state)
							.setEmoji("👤"),
						new ButtonBuilder()
							.setStyle(licenseInfo["Ip-Cap"] === null ? ButtonStyle.Secondary : ButtonStyle.Primary)
							.setCustomId("ip-cap")
							.setLabel("Ip-Cap")
							.setDisabled(state)
							.setEmoji("📡"),
						new ButtonBuilder()
							.setStyle(licenseInfo["Expire at"] === null ? ButtonStyle.Secondary : ButtonStyle.Primary)
							.setCustomId("expire")
							.setLabel("Expire")
							.setDisabled(state)
							.setEmoji("🕙"),
						new ButtonBuilder()
							.setStyle(licenseInfo["Hwid-Cap"] === null ? ButtonStyle.Secondary : ButtonStyle.Primary)
					...
					new ActionRowBuilder<ButtonBuilder>().addComponents(
						new ButtonBuilder()
							.setStyle(licenseInfo.Reason === null ? ButtonStyle.Secondary : ButtonStyle.Primary)
							.setCustomId("reason")
							.setLabel("Reason")
							.setDisabled(state)
							.setEmoji("✏️"),
				...
				if (licenseInfo.Product && licenseInfo.User && licenseInfo["Ip-Cap"] !== null) {
			...
			const mainEmbed = (finish = false) => {
				const fields = [
					{
						...
						value: Object.entries(licenseInfo)
							...
							.map(([k, v]) => {
								if (k === "Expire at") return `> ${k}: ${v === null ? "**Never**" : dateToTimestamp(licenseInfo["Expire at"])}`;
								return `> ${k}: **${v.name ? v.name : v === null ? "Unknown" : v}**`;
							}).join("\n")
					}
				];
				...
				return [
					new EmbedBuilder()
						...
						.setColor(finish ? "Green" : client.config.GeneralSettings.EmbedColor)
						.addFields(fields)
						.setTimestamp()
				];
			};
			...
			collector.on("collect", async (int) => {
					...
					try {
						...
						if (!products.length) {
							await int.editReply({
								embeds: [
									new EmbedBuilder()
										.setTitle("You need create a product using -/product create")
										.setColor("Red")
								],
								components: []
							});
				...
				if (int.customId === "save") {
					...
					const licenseMember = interaction.guild.members.cache.get(license.discord_id);
					await licenseMember.roles.add(licenseInfo.Product.role).catch(e => e);

					if (client.config.GeneralSettings.CustomerRole?.length > 3) {
						await licenseMember.roles.add(client.config.GeneralSettings.CustomerRole)
							.catch(e => e);
					}

					if (licenseMember.dmChannel) {
						await licenseMember.send({
							embeds: [
								new EmbedBuilder()
									.setAuthor({
									...
									.setDescription("Sharing this key with anyone will result in your license being **permanently** disabled and your acces to the product **removed**.")
									.addFields([
										{
											name: "Product",
							...
							.addComponents(
								new ActionRowBuilder<TextInputBuilder>().addComponents(
									new TextInputBuilder()
										.setPlaceholder("How many IP's will the user be allowed to use his license?")
					...
					await int.showModal(
						new ModalBuilder()
							...
							.addComponents(
								new ActionRowBuilder<TextInputBuilder>().addComponents(
									new TextInputBuilder()
										.setPlaceholder("How many days will this license last? use 0 for never")
										.setStyle(TextInputStyle.Short)
										.setCustomId("data")
										.setLabel("Days")
										.setRequired(true)
					...
					await int.showModal(
						new ModalBuilder()
							...
							.addComponents(
								new ActionRowBuilder<TextInputBuilder>().addComponents(
									new TextInputBuilder()
										.setPlaceholder("How many HWID's will the user be allowed to use his license?")
										.setStyle(TextInputStyle.Short)
										.setCustomId("data")
										.setLabel("Hwid-Cap")
										.setRequired(true)
					...
					embeds: [
						new EmbedBuilder()
							...
							.addFields({
								...
								value: ">>> Hey, you don't have any license to list, create one using the command -/license create"
							})
							.setColor("Red")
							.setTimestamp()
					]
				...
				embeds.push(
					new EmbedBuilder()
						...
						.addFields(
							{
								name: "• License Information:",
								value: `>>> License Key: **${decrypt(license.license).slice(0, 100)}**\nReason: **${(license.reason || "None").slice(0, 100)}**\nProduct: **${license.product_name.slice(0, 100)}**\nTotal Requests: **${license.total_requests}**\nLatest IP: **${(license.latest_ip || "Unknown").slice(0, 100)}**\nLatest HWID: **${(license.latest_hwid || "Unknown").slice(0, 100)}**\nExpire at: ${license.expires_date ? dateToTimestamp(license.expires_date) : "**Never**"}\nCreated at: ${dateToTimestamp(license.createdAt).slice(0, 100)}`
							},
							{
								name: "• User Information",
								value: `>>> User: <@!${user.id}> (${user.tag.slice(0, 100)})\nUser ID: **${user.id.slice(0, 100)}**`
							}
						)
						.setColor(client.config.GeneralSettings.EmbedColor)
						.setTimestamp()
					...
					embeds: [
						new EmbedBuilder()
							...
							.addFields({
								...
								value: ">>> Hey, this license does not exist, check that you have sent the license correctly, remember that we are case sensitive"
							})
							.setColor("Red")
							.setTimestamp()
					]
				...
				return interaction.reply({
					embeds: [
						new EmbedBuilder()
							...
							.addFields({
								name: "• User not found",
								value: ">>> Please give a user or user ID to delete all licenses from that user."
							})
							.setColor("Red")
							.setTimestamp()
					]
					...
					embeds: [
						new EmbedBuilder()
							...
							.addFields({
								...
								value: ">>> Hey, this user does not have any licenses, check that you have sent the user correctly."
							})
							.setColor("Red")
							.setTimestamp()
					]
					...
					embeds: [
						new EmbedBuilder()
							...
							.addFields(
								{
									name: "• License Information:",
									value: `>>> Licenses: **${licenses.length}**\nProduct: **${licenses[0].product_name}**\nCreated at: ${dateToTimestamp(licenses[0].createdAt)}\nCreated by: <@!${licenses[0].createdBy}>`
								},
								{
									name: "• User Information:",
									value: `>>> User: <@!${user.id}> (${user.tag})\nUser ID: **${user.id}**`
			...
			const message = await interaction.reply({
				embeds: [
					new EmbedBuilder()
						.setTitle(`Do you want to restart the ip's or hwid's of the license\n**${license}**?`)
						.setColor("Orange")
				],
			...
			client.emit("licenseClear", licenseData, interaction.user, int.customId === "hwid" ? "HWID" : "IP");
			return interaction.editReply({
				embeds: [
					new EmbedBuilder()
						.setAuthor({
							name: `License Cleared (${int.customId === "hwid" ? "HWID" : "IP"})`,
							iconURL: "https://cdn-icons-png.flaticon.com/64/1828/1828506.png"
						})
						.addFields([
							{
			...
			type queryTypes = "ip_list" | "hwid_list" | "license" | "discord_id" | "reason";
			const ephemeral = !interaction.options.getBoolean("visible");
				...
				embeds.push(
					new EmbedBuilder()
						...
						.addFields(
							{
								name: "• License Information:",
								value: `>>> License Key: **${decrypt(license.license)}**\nReason: **${license.reason || "None"}**\nProduct: **${license.product_name}**\nTotal Requests: **${license.total_requests}**\nLatest IP: **${license.latest_ip || "Unknown"}**\nLatest HWID: **${license.latest_hwid || "Unknown"}**\nExpire at: ${license.expires_date ? dateToTimestamp(license.expires_date) : "**Never**"}\nCreated at: ${dateToTimestamp(license.createdAt)}`
							},
							{
								name: "• User Information",
								value: `>>> User: <@!${user.id}> (${user.tag})\nUser ID: **${user.id}**`,
							...
							{
								name: `• IP Information (${license.ip_list.length}/${license.ip_cap || "∞"})`,
								value: `>>> ${license.ip_list.map(ip => `• ${ip}`).join("\n") || "• No IP's registered."}`,
								inline: true
							},
							{
								name: `• HWID Information (${license.hwid_list.length}/${license.hwid_cap || "∞"})`,
								value: `>>> ${license.hwid_list.map(ip => `• ${ip}`).join("\n") || "• No HWID's registered."}`,
								inline: true
							},
							{
								name: "\u200b",
			...
			if (!licenseData) {
				return interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setTitle("This license does not exist, check that you have sent the license correctly")
							.setColor("Red")
					],
					ephemeral: true
				});
			...
			const querySearch = {
				...
				expires_date: "Submit the new number of days the license will have before it expires.\n(use **0** for a lifetime license)"
			};
			...
			await messages.first().delete().catch(e => e);
			...
			if (int.customId === "discord_id") {
				const query = cleanContent.replace("<", "").replace("@", "").replace(">", "").replace("!", "");
			...
			const newLicenseData = await LicenseModel.findOne({ license: licenseData.license });
				...
				embeds: [
					new EmbedBuilder()
						...
						.addFields(
							{
								name: "• License Information:",
								value: `>>> License Key: **${decrypt(newLicenseData.license)}**\nReason: **${newLicenseData.reason || "None"}**\nProduct: **${newLicenseData.product_name}**\nTotal Requests: **${newLicenseData.total_requests}**\nLatest IP: **${newLicenseData.latest_ip || "Unknown"}**\nLatest HWID: **${newLicenseData.latest_hwid || "Unknown"}**\nExpire at: ${newLicenseData.expires_date ? dateToTimestamp(newLicenseData.expires_date) : "**Never**"}\nCreated at: ${dateToTimestamp(newLicenseData.createdAt)}`
							},
							{
								name: "• User Information",
								value: `>>> User: <@!${user.id}> (${user.tag})\nUser ID: **${user.id}**`
